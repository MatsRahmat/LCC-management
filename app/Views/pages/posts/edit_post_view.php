...
<section class="space-y-2">
    <?= form_open_multipart('a/admin/posts/update/' . $post['id'], ['class' => '', 'id' => 'form']) ?>
    <div>
        <label for="title">Judul</label>
        <textarea name="title" id="title" class="w-full border focus:outline-blue-500 focus:bg-slate-100 transition-colors border-black rounded-md p-2"><?= old('title') ?? esc($post['title']) ??  null ?></textarea>
            ...
            reader.onload = function(e) {
                const div = $('<div>')
                    ...
                    .css({
                        flex: "1 1 200px",
                        'margin-bottom': '10px'
                    }); // .attr('style', 'flex:1 1 200px; margin-bottom:10px;')
                const img = $('<img>')
