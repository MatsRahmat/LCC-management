            ...
            <?php foreach ($data as $attch): ?>
                <div data-card="adasd" class="border border-black rounded-md flex justify-between p-1">
                    ...
                    <div class="flex-none border-l border-black flex gap-2 py-1 px-2 items-center">
                        ...
                        <button class="border-none bg-red-400 rounded-md" data-delete="<?= $attch['id'] ?>">
                            <div class="flex gap-1 items-center">
...
<script>
    $(document).ready(function() {
        ...
        $('button[data-delete]').on('click', async function() {
            const confirm = await Prompts.confirm('Apakah anda yakin ingin menghapus data ini?');
            if (confirm) {
