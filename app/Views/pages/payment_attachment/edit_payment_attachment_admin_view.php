...
<section class="p-2">
    ...
    <?= form_open_multipart($page['page_path'] . '/update/' . $attach['id'], ['class' => 'class1 class2 class3s', 'id' => 'form-login', 'data-form-login' => 'adasdsadasd']) ?>
    <div>
        <label for="desc">Keterangan</label>
        <textarea name="desc" id="desc" class="w-full border border-black rounded-md p-2"><?= old('desc') ?? null ?><?= $attach['desc'] ?></textarea>
    ...
    <div>
        ...
        <div class="border border-black rounded-md w-full flex justify-between items-center">
            ...
            <div class="flex-1 border-l border-black px-1">
                <p data-section="info-attachment"><?= $attach['original_name'] ?> - <?= $attach['size_formated'] ?></p>
            </div>
        </div>
